...
# 적(스프라이트) 불러오기
enemy = pygame.image.load('studyPython/pygames_project/pygame_basic/enemy.png')
# 적 크기 가져오기
...
while running:
    ...
    character_rect = character.get_rect() # 고정된 현재 위치를 가져온다.
    character_rect.left = character_x_pos # 변화된 값을 반영한다.
    character_rect.top = character_y_pos

    enemy_rect = enemy.get_rect() # 고정된 현재 위치를 가져온다.
    enemy_rect.left = enemy_x_pos # 변화된 값을 반영한다.
    enemy_rect.top = enemy_y_pos

    # 충돌 체크 - colliderect : 사각형을 기준으로 충돌 여부 파악
    ...
    # 타이머 집어 넣기
    # 경과 시간 계산 - 경과 시간(ms)을 1000으로 나누어서 초(s) 단위로 표시
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000
    # 총 시간에서 1씩 줄어든다. int는 소수점을 버려서 10, 9초로 표시.
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255))
    # game_font.render(출력할 글자, True, 글자 색상(r, g, b))
