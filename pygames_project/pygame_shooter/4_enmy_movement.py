...
while running:
    ...
    # 적이 화면 밖으로 사라지면 새로운 적이 나온다.
    if enemy_y_pos > screen_height or enemy_y_pos < 0 or enemy_x_pos > screen_width or enemy_x_pos < 0:
        enemy_x_pos = random.randint((screen_width / 2), screen_width - enemy_width) # randint(시작, 끝)
        enemy_y_pos = random.randint(0, screen_height - enemy_height) # randint(시작, 끝)
